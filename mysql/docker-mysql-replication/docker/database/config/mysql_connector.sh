...
echo "stopping slave in SLAVE MYSQL $MYSQL_SLAVE_ADDRESS:$MYSQL_SLAVE_PORT xxx"
...
mysql --host $MYSQL_MASTER_ADDRESS --port $MYSQL_MASTER_PORT -uroot -p$MYSQL_MASTER_ROOT_PASSWORD -AN -e "stop slave;";
mysql --host $MYSQL_MASTER_ADDRESS --port $MYSQL_MASTER_PORT -uroot -p$MYSQL_MASTER_ROOT_PASSWORD -AN -e "reset slave all;";
...
mysql --host $MYSQL_SLAVE_ADDRESS  --port $MYSQL_SLAVE_PORT -uroot -p$MYSQL_SLAVE_ROOT_PASSWORD -AN -e "create user '$MYSQL_REPLICATION_USER'@'%';"
mysql --host $MYSQL_SLAVE_ADDRESS --port $MYSQL_SLAVE_PORT -uroot -p$MYSQL_SLAVE_ROOT_PASSWORD -AN -e "grant replication slave on *.* to '$MYSQL_REPLICATION_USER'@'%' identified by '$MYSQL_REPLICATION_PASSWORD';"
mysql --host $MYSQL_SLAVE_ADDRESS --port $MYSQL_SLAVE_PORT -uroot -p$MYSQL_SLAVE_ROOT_PASSWORD -AN -e "flush privileges;"


echo "getting MASTER MYSQL config"
Master_Position="$(mysql --host $MYSQL_MASTER_ADDRESS --port $MYSQL_MASTER_PORT -uroot -p$MYSQL_MASTER_ROOT_PASSWORD -e 'show master status \G' | grep Position | grep -o '[0-9]*')"
...
Slave_Position="$(mysql --host $MYSQL_SLAVE_ADDRESS --port $MYSQL_SLAVE_PORT -uroot -p$MYSQL_SLAVE_ROOT_PASSWORD -e 'show master status \G' | grep Position | grep -o '[0-9]*')"
Slave_File="$(mysql --host $MYSQL_SLAVE_ADDRESS --port $MYSQL_SLAVE_PORT -uroot -p$MYSQL_SLAVE_ROOT_PASSWORD -e 'show master status \G' | grep File | sed -n -e 's/^.*: //p')"
...
mysql --host $MYSQL_MASTER_ADDRESS --port $MYSQL_MASTER_PORT -uroot -p$MYSQL_MASTER_ROOT_PASSWORD -AN -e"change master to master_host='$MYSQL_SLAVE_ADDRESS',master_port=$MYSQL_SLAVE_PORT,master_user='$MYSQL_REPLICATION_USER',master_password='$MYSQL_REPLICATION_PASSWORD',master_log_file='$Slave_File',master_log_pos=$Slave_Position;"
...
mysql --host $MYSQL_MASTER_ADDRESS --port $MYSQL_MASTER_PORT -uroot -p$MYSQL_MASTER_ROOT_PASSWORD -AN -e "start slave;"
mysql --host $MYSQL_MASTER_ADDRESS --port $MYSQL_MASTER_PORT -uroot -p$MYSQL_MASTER_ROOT_PASSWORD -e "show slave status \G;"
